...
for link in soup.findAll('a', attrs={'href': re.compile("./?page=productfeed&id=")}):
...
pr = str(produit_1).replace('<div id="productname" style="font-weight: 600;font-size: 1.4em;padding: 5px;padding-top: 0px;margin-top: 0px;text-transform: uppercase;">', '')
produit_2 = pr.replace('</div>', '')
px = str(prix_1).replace('<div id="productprice" style="font-weight: 600;font-size: 1.4em;padding: 5px;padding-top: 0px;margin-top: 0px;text-transform: uppercase;float:right;color: #109800;">', '')
...
try:
    ...
    print("Message envoyé avec succé, code {}.".format(result.status_code))

# Redémarrer le script

while 1:
    os.system("python3 gpu_tracker.py")
    print("Restarting...")
